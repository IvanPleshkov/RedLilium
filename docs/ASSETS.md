...
## Статус
Ревизия 4. Фазы 1–2 и §9 (`AssetRef` + sync-система) реализованы: меши и
материал-инстансы биндятся через `AssetRef`, менеджеры — источник истины с
`generation`. Следующий шаг — хот-релоад (`ChangedAssets` → `invalidate`).
