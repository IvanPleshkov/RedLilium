...
## Статус
Дизайн, ревизия 3 (VFS-пара; без второго refcount — переиспользуем `Arc<Mesh>`;
аплоад только через граф, прямой путь на удаление; residency ≥1 кадр). Реализацию
не начинали — можно приступать с фазы 1.
