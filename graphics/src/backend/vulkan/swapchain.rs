...
pub struct VulkanSwapchainAcquireResult {
    ...
    pub suboptimal: bool,
    /// Whether the present should be id-tagged and paced with
    /// `vkWaitForPresentKHR` (see [`VulkanSwapchain::present_wait`]).
...
pub fn present_vulkan_frame(
    ...
    // Present pacing: tag this present with a monotonic non-zero id (the
    // Surface's frame counter is monotonic across swapchain recreations,
    // which `vkWaitForPresentKHR`'s watermark semantics accept).
    let present_ids = [frame_index + 1];
    ...
    // Skipped when this present did not reach the queue (OUT_OF_DATE et al).
    // The timeout is a safety valve (compositor stall, occluded window) —
    // pacing then degrades gracefully instead of failing the frame.
    if present_wait
        && frame_index > 0
        && matches!(result, Ok(_) | Err(vk::Result::SUBOPTIMAL_KHR))
        && let Some(wait_loader) = vulkan_backend.present_wait_loader()
    {
        const PRESENT_WAIT_TIMEOUT_NS: u64 = 100_000_000; // 100 ms
        let wait_result = unsafe {
            wait_loader.wait_for_present(swapchain, frame_index, PRESENT_WAIT_TIMEOUT_NS)
        };
        match wait_result {
            Ok(_) => {}
