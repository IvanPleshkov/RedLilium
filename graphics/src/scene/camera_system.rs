...
impl CameraRenderContext {
    ...
    pub fn setup_forward_graph(&mut self) {
        ...
        if self.camera.clear_color.is_some() {
            let _clear_pass = self.graph.add_graphics_pass(GraphicsPass::new(format!(
                "camera_{}_clear",
        ...
        let _geometry_pass = self.graph.add_graphics_pass(GraphicsPass::new(format!(
            "camera_{}_geometry",
            self.camera.entity_id
        )));
        ...
        if !self.visible_masked.is_empty() {
            let _masked_pass = self.graph.add_graphics_pass(GraphicsPass::new(format!(
                "camera_{}_masked",
                self.camera.entity_id
            )));
        }
        ...
        if !self.visible_transparent.is_empty() {
            let _transparent_pass = self.graph.add_graphics_pass(GraphicsPass::new(format!(
                "camera_{}_transparent",
                self.camera.entity_id
            )));
        }
    }
